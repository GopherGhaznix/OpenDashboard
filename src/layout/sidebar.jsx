...
const Sidebar = () => {
    ...
    return (
        <aside style={{ borderRight: '3px solid #262630' }}
            className={`transition-all duration-300 pb-4 ${isOpen ? "w-72" : "w-18"} h-full flex flex-col justify-between`}
        >
            {/* Header Section */}
            <div className="p-4 pe-0">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center">
                        ...
                        />
                        <h2 className={`text-base font-semibold text-gray-300 transition-opacity overflow-hidden text-nowrap duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                            {user.name}
                        </h2>
                    </div>
                    <div className={`flex overflow-hidden text-nowrap ${isOpen ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}>
                        ...
                        {menuItems.map(({ label, icon, link, subItems, indicator, separate }, index) => (
                            <li key={index} className={`mb-4 text-gray-400 ${separate && "border-t border-[#2d2d39] border-b py-4"} ${index + 1 === menuItems.length && "border-[#2d2d39] border-b pb-4"}`}>
                                <NavLink to={link} className='flex justify-between sideitemHoverEffect rounded-lg p-1.5'>
                                    <a className="flex items-center gap-3 hover:text-white font-bold">
                                        <span>{icon}</span>
                                        <span className={`transition-opacity duration-300 overflow-hidden text-nowrap ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                                            {label}
                                        </span>
                                    </a>
                                ...
                                {subItems && (
                                    <ul className={`ml-9 my-2 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'} overflow-hidden`}>
            ...
            <div className="px-4 ">
                <nav>
                    <ul>
                        {footerItems.slice(0, 3).map(({ label, icon, link }, index) => (
